...
function titleCase(str) { // create a new function that accepts a single argument, being a user-passed string of alphabetical characters
  return str // return a copy of the mutated string...
    ...
    .map(val => val.replace(val.charAt(0), val.charAt(0).toUpperCase())) //...and mapping the returned value of an anonymous function that accepts a value argument which leverages the .replace() method to look at the first character of each split word within the string and apply the .touppercase() method on each of them...
    .join(" ");
}
